...
Let's say that for the purposes of debugging, you want to display the contents a server-side object in the browser.  You could write something like this:
...
If you wish to generate a complete HTML page instead of just a `DIV` fragment, your code would look something like this:
...
Multiple objects can be transformed into separate `DIV` elements that are then returned with a parent `DIV`:
...
Hardcoded functions that return the contents of the NodeJS `process` and `global` objects respectively.
...
| `isExpandable` | `Boolean` | Returns `true` only for those objects whose content is considered expandable.<br>Currently, this applies only to `Object`, `Array` or `Map`.
...
| Name | Return Type | Description
|---|---|---|
| `create_content` | `String` | Deprecated in favour of `show_objects` instead
