...
The `create_content` function transforms a JavaScript object into an HTML table.  If any property of this object is expandable (I.E. is of type `Map`, `Object` or `Array`), then this property value will itself be transformed into a nested HTML table.
...
Let's say that for the purposes of debugging, you want to display the contents of one or more server-side objects in the browser.  You could write something like this:

```javascript
var bfu = require('basic-formatting-utils')

var show_objects = tv_array        => bfu.create_content(tv_array)
var show_object  = (desc, obj_val) => show_objects([{title: desc, value: obj_val}])
...
More fully, if you wish to display a complete HTML page instead of just a `DIV` fragment, your code would look something like this:

```javascript
var bfu = require('basic-formatting-utils')

var show_objects = tv_array =>
  bfu.as_html([]
  , bfu.as_body([]
    , bfu.create_content(tv_array)
    )
  )
...
Then the `show_objects` function exposed above should be passed an array of title/value objects:

```javascript
var request = ... /* Some HTTP request object */
var event   = ... /* Some HTTP event that triggered the execution of this current unit of code */
...
Or, if you wish simply to make use of the provided convenience functions:
...
// *****************************************************************************
module.exports = {
  show_nodejs_process: bfu.show_nodejs_process
  show_nodejs_global : bfu.show_nodejs_global
}
```
