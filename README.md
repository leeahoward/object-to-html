...
The `show_object` function transforms a JavaScript object into an HTML table.  If any property of this object is expandable (I.E. is of type `Map`, `Object` or `Array`), then this property value will itself be transformed into a nested HTML table.
...
Enclose the call to `show_object` within calls to `as_body` and `as_html` in order to generate a complete HTML page instead of just a `<DIV>` fragment:
