...
var set_depth_limit = (lim) =>
  (depth_limit = isNumeric(lim) && lim >= 1 ? lim : depth_limit);
...
var image_src_data = [
  ...
  "",
  `var arrow_right_src = "data:image/png;base64, ${fs
    .readFileSync(__dirname + "/arrow_right.b64.txt")
    .toString()}";`,
  `var arrow_down_src  = "data:image/png;base64, ${fs
    .readFileSync(__dirname + "/arrow_down.b64.txt")
    .toString()}";`,
  "",
...
var typeOf = (typeName) =>
  Object.prototype.toString.apply(typeName).slice(8).slice(0, -1);

// Partial function that creates a function to check for a specific data type
var isOfType = (targetType) => (someObject) =>
  typeOf(someObject) === targetType;
...
var make_tag = (tag_name, props_array) =>
  ((noProperties) =>
    `<${tag_name}${noProperties ? "" : " " + props_array.join(" ")}>`)(
...
var as_html_el = (tag_name) => (propsArray, val) =>
  ((openingTag, tagIsEmpty) =>
    `${openingTag}${tagIsEmpty ? "" : `${val}</${tag_name}>`}`)(
    make_tag(tag_name, propsArray),
    isEmptyElement(tag_name) || isNullOrUndef(val)
  );
...
var make_table_rows_from_obj = (obj_name, obj, col1txt, depth) => {
  ...
  if (sizeOf(obj) > 0) {
    ...
    if (isArray(obj)) {
      acc.push(
        obj
          .map((el, idx) => make_table_row_from_prop(obj_name, idx, el, depth))
          .join("")
      );
      ...
      acc.push(
        prop_names
          .map((prop_name) =>
            make_table_row_from_prop(obj_name, prop_name, obj[prop_name], depth)
          )
          .join("")
      );
    } else if (isMap(obj)) {
      ...
      obj.forEach((val) =>
        acc.push(make_table_row_from_prop(obj_name, val, null, depth, true))
      );
    }
...
var make_collapsible_div = (div_name, table_rows, depth) =>
  as_div(
    [`id="${div_name}-content"`, depth === 0 ? "" : "style='display:true'"],
...
var make_table_row_from_prop = (
  ...
  if (suppress_fns && isFunction(prop_value)) {
    ...
    var type_col =
      isExpandable(prop_value) && sizeOf(prop_value) > 0 && depth < depth_limit
...
var render_value = (enum_arg, enum_name, depth) =>
  ...
  isExpandable(enum_arg)
    ? // Yup, so transform it into a table.
      make_collapsible_div(
        ...
        make_table_rows_from_obj(
          enum_name,
          enum_arg,
          columnHeadings[typeOf(enum_arg)],
          depth
        ),
        depth
      )
    ...
    : enum_arg.replaceAll
    ? enum_arg.replaceAll("<", "&lt;").replaceAll(">", "&gt;")
    : enum_arg;
  ...
  isArray(tvArray) && tvArray.length > 0
    ? as_div(
        ...
        [
          as_style(
            [],
            fs.readFileSync(__dirname + "/bfu-style.css").toString()
          ),
          // Transform one or more objects
          tvArray
            .map((el) =>
              create_content_table(
                `${el.title}${suppress_fns ? " (Functions suppressed)" : ""}`,
                el.value
              )
            )
            .join(""),
          ...
          as_script(
            ...
            fs.readFileSync(__dirname + "/expand_collapse.js").toString()
          ),
        ].join("")
      )
...
var arrow_content = (obj_type, arrow_type) =>
  [obj_type, as_button(["type='button'"], arrow_type)].join("");
...
var expand_button_div = (obj_name, obj_type) =>
  as_div(
    arrow_properties(obj_name, "right", "expand", true),
    arrow_content(obj_type, arrow_right)
  );

var collapse_button_div = (obj_name, obj_type) =>
  as_div(
    arrow_properties(obj_name, "down", "collapse", false),
    arrow_content(obj_type, arrow_down)
  );

// *********************************************************************************************************************
// Partial function to create a date/time stamp for a given timezone offset in minutes
var datetime_by_timezone = (offset) => (date) =>
...
module.exports = {
  ...
  show_nodejs_global: () =>
    create_content([{ title: "NodeJS global", value: global }]),
  show_nodejs_process: () =>
    create_content([{ title: "NodeJS process", value: process }]),
};
