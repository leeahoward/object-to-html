...
const static_script_name = "basic-formatting-utils-script.js"



// *********************************************************************************************************************
// This coding is dumped to a local file that is then requested by the generated HTML
// This is needed to avoid duplicating the Base64 encoded image data every time an IMG element is generated
const src_code = [
...
// Write the above static JavaScript to a local file
fs.writeFileSync(static_script_name, src_code)
...
var value_to_str = (obj_name, val, depth) =>
  (valType =>
    ...
    : valType === "Function"
      ...
      : valType === "Array"
        // Yup, so is display the contents of the array
        ...
        : valType === "Map"
          ...
          // So now we've eliminated all the datatypes where some sort of special formatting is needed
          // Therefore, we'll implicitly call this value's toString() function and hope it returns something useful...
          : val
  )
  // Pass the current value's datatype as an argument to the above inner (anonymous) function
  (typeOf(val))
...
var array_to_str = (arr, arr_name, depth) =>
  ...
  ? arr.length
    ...
    // ? as_div(
    //     [`id="${arr_name}-content"`, "style=\"display:none\""]
    //   , arr.map(el => value_to_str(arr_name, el, depth+1)).join("<br>")
    //   )
    ? arr.map(el => value_to_str(arr_name, el, depth+1)).join("<br>")
    // Nope - nothing to see here, move along...
    : "[]"
  // Yes, so shown that the array has content, but we're just not going to display it
  : "[...]"
...
var object_to_table = (obj_arg, obj_name, depth) => {
  ...
  // Does the object contain anything we can display?
    ...
    if (Object.keys(obj_arg).length > 0) {
        ...
        if (obj_type === "Array" || obj_type === "Object" || obj_type === "Map") {
          if (prop_count > 0) {
            if (depth <= depth_limit) {
              ...
              type_col  = as_td([], [expand_button_div(this_prop_name, obj_type), collapse_button_div(this_prop_name, obj_type)].join(""))
              value_col = as_td([], value_to_str(this_prop_name, obj_arg[key], depth))
            }
            else {
              // Display an object, array or map with suppressed content due to recursion depth limit and no
              // expand/collapse buttons
              type_col  = as_td([], obj_type)
              value_col = as_td([], obj_type === "Array" ? "[...]" : "{...}")
            }
          }
          else {
      ...
      return_val = as_table([`id=${obj_name}-content`], acc.join(""))
    }
    else {
...
var create_content_table = (hdr, obj) =>
  (obj_name =>
    as_div(
      ["class='content'", `id='${obj_name}-content'`],
      [ as_h2([], hdr), object_to_table(obj, obj_name)].join("")
    )
  )
  (typeOf(obj))
...
var create_content = nvArray => 
  ...
  ? as_div([]
    , [ as_style([], content_table_style)
      ...
      , as_script(["type=\"text/javascript\"", `src="./${static_script_name}"`],"")
      ].join("")
    )
  : ""
...
// value every time one of these icons is generated.  Instead, the src of each icon will be added by the static
// JavaScript code dumped into the local file "script.js"
...
var expand_button_div = (obj_name, obj_type) =>
  as_div(
    ["class=\"arrow-right\"", `id="${obj_name}-arrow-right"`, `onclick="expand('${obj_name}')"`]
  , [obj_type, as_button(["type=\"button\""], arrow_right)].join("")
  )

var collapse_button_div = (obj_name, obj_type) =>
  as_div(
    ["class=\"arrow-down\"", `id="${obj_name}-arrow-down"`, "style=\"display:none\"", `onclick="collapse('${obj_name}')"`]
